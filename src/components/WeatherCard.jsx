...
return (
    <section className='weatherCard'>
        <h1 className='weatherCard_title'>Weather for Today</h1>
        <h2 className='weatherCard__place'>{weather?.name}, {weather?.sys.country}</h2>
        <div className='weatherCard__img'>
            <img src={`/images/${weather?.weather[0].icon}.png`}/>
        </div>
        <h3 className='weatherCard__temp'>{isCelcius ? temps?.celcius + " °C " : temps?.fahrenheit + " °F " } </h3>
